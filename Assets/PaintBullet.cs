...
public class PaintBullet : MonoBehaviour
{
    ...
    private Color targetColor; // The color we represent
    ...
    void Start()
    {
        ...
        // Color is now set in Initialise, so we just apply it here to be safe
        ColorChildren();
    }
    ...
    public void Initialise(Collider2D shooterCollider, bool isPlayerTeam)
    {
        ...
        // Apply color immediately so it doesn't flash the wrong color for 1 frame
        ColorChildren();
    }
    ...
    void CheckTileUnderneath()
    {
        ...
        // If the tile is NOT our team's color, paint it
        if (!IsSimilarColor(tileColor, targetColor))
        {
            // IMPORTANT: We register True for Player, False for Enemy
